...
%let geo_level = year state puma;
%let geolist ="year", "state", "puma";
%let lastgeo= puma;
%let datas=housing;
...
create table hhousing_&year. as
select 
   a.*
  ,b.puma12  as puma12
  ,b.afact1 as afact1
...
%LET CAT_housing=ACREHOUS BEDROOMS BUILTYR2 FARM FARMPROD  FOODSTMP  FUELHEAT HHTYPE 
						 KITCHEN LINGISOL MORTGAG2 MORTGAGE 
						 PHONE PLUMBING
						PROPTX99 RENTMEAL  ROOMS 
						TAXINCL  UNITSSTR VEHICLES;

%LET NUME_housing=  CONDOFEE COSTELEC  COSTFUEL OWNCOST 
                       COSTGAS  COSTWATR MORTAMT1 MORTAMT2
						PROPINSR RENT RENTGRS VALUEH;


; 
%LET CROSS_CAT_housing = ACREHOUS BEDROOMS1 BUILTYR1 FARM  FOODSTMP FUELHEAT HHTYPE 
						 KITCHEN LINGISOL MORTGAG2 MORTGAGE  PHONE PLUMBING
						PROPTX1 RENTMEAL ROOMS1 
						TAXINCL  UNITSSTR VEHICLES1;


%LET CROSS_NUM_housing=  CONDOFEE COSTELEC  COSTFUEL OWNCOST
                         COSTGAS  COSTWATR MORTAMT1 MORTAMT2
						PROPINSR RENT RENTGRS VALUEH;



%LET CAT_CROSS_housing1= FARM  FOODSTMP  HHTYPE  MORTGAGE   UNITSSTR  ;
...
%do i=2005 %to 2015;
 data householda_&i.;
    set ipums.acs_hfile( where=(rectype ="H" and year=&i.));

    if (gq  = 1 or gq=2 or gq=0);
...
keep year STATE puma hhwt &cat_housing. &nume_housing. 
